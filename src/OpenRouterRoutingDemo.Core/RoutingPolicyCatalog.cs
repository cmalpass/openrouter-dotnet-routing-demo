    ...
    public static RoutingPolicyCatalog CreateDefault(string? freeModel = null)
    {
        ...
        if (!freeModel.EndsWith(":free", StringComparison.OrdinalIgnoreCase))
        {
            throw new ArgumentException(
                "The free routing policy must use an OpenRouter model with the :free variant.",
                nameof(freeModel));
        }
        ...
        [
            ...
            new RoutingPolicy(
                name: "free",
                description: "Use a zero-price model for low-stakes development and live smoke tests.",
                models: [freeModel],
                provider: new ProviderRoutingOptions
                {
                    AllowFallbacks = true,
